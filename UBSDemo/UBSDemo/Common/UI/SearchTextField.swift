...
@objc public protocol FavoriteDataSource {
...
open class SearchTextField: UITextField {
    ...
    open weak var favoriteDatasource: FavoriteDataSource?
    ...
    fileprivate func filter(forceShowAll addAll: Bool) {
        ...
        for i in 0 ..< filterDataSource.count {
            ...
            if !inlineMode {
                ...
                let titleFilterRange = (item.title.prefix(text!.count) as NSString).range(of: text!, options: comparisonOptions)
...
extension SearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let isFavorite = favoriteDatasource?.isFavorite(filteredResults[(indexPath as NSIndexPath).row].title)
        cell!.imageView?.image = isFavorite! ? UIImage(named: "ic_favorite") : nil
